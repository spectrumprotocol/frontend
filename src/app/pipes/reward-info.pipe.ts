...
export class RewardInfoPipe implements PipeTransform {
  ...
  transform(rewardInfo: RewardInfoResponseItem): string {
    ...
    if (+rewardInfo.auto_bond_amount >= 10) {
      str += this.unit.transform(+rewardInfo.auto_bond_amount, '1.0-6') + ' in auto compound pool';
    }
    if (+rewardInfo.stake_bond_amount >= 10) {
      if (str) {
        str += ' + ';
      }
      str += this.unit.transform(+rewardInfo.stake_bond_amount, '1.0-6') + ' in auto stake pool';
    }
    return str;
  }
