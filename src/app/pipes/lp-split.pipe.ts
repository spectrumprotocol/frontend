...
export class LpSplitPipe implements PipeTransform {
  ...
  transform(lp: number, poolResponse: PoolResponse, symbol: string, pairSymbol: string, digitsInfo?: string): string {
    ...
    if (poolResponse.assets[0].info.native_token) {
      return `${this.decimalPipe.transform(amount2, digitsInfo)} ${symbol} + ${this.decimalPipe.transform(amount1, digitsInfo)} ${pairSymbol}`;
    } else {
      return `${this.decimalPipe.transform(amount1, digitsInfo)} ${symbol} + ${this.decimalPipe.transform(amount2, digitsInfo)} ${pairSymbol}`;
    }
  }
