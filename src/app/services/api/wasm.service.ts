...
export class WasmService {
  ...
  query(contract: string, msg: object): Promise<any> {
    if (this.terrajs.USE_NEW_BASE64_API) {
      return this.terrajs.get(`cosmwasm/wasm/v1/contract/${contract}/smart/` + Buffer.from(JSON.stringify(msg), 'utf-8').toString('base64')).catch(e => {
        console.error('query error::', contract, msg, e);
      });
