...
export class VaultDialogComponent implements OnInit, OnDestroy {
    ...
    this.heightChanged = this.terrajs.heightChanged.subscribe(async () => {
      if (this.terrajs.isConnected) {
        if (this.vault.poolInfo.farmType === 'LP'){
          ...
          const tasks: Promise<any>[] = [];
          tasks.push(this.info.refreshTokenBalance(this.vault.poolInfo.baseTokenContractOrNative));
          await Promise.all(tasks);
        }
      }
    });
  ...
  async doDeposit() {
    ...
    if (this.depositMode === 'tokentoken') {
      ...
      const msgs = [
        ...
        new MsgExecuteContract(this.terrajs.address, this.vault.poolInfo.baseTokenContractOrNative, {
          send: {
  ...
  async depositLPChanged(forced: boolean, event?: any) {
    ...
    const depositTVL = new BigNumber(this.lpBalancePipe.transform(times(this.depositLPAmtLP, CONFIG.UNIT) ?? '0', this.info.poolResponses, this.vault.poolInfo.baseTokenContractOrNative));
